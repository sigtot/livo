...
Smoother::Smoother()
    : fixedLagSmoother(7.0, ISAM2Params()), isam2(getIsam2Params()) {}

void Smoother::update(const shared_ptr<Frame>& frame) {
  graph.resize(0);
  Values newValues;

  /* TODO Useful for debugging, but remove after refactoring dupe removal code
  in FeatureExtractor for (int i = 0; i < frame->keyPointObservations.size();
  ++i) { for (int j = 0; j < frame->keyPointObservations.size(); ++j) { if (i ==
  j) { continue;
          }
          ...
          if (landmark1 && landmark2 && landmark1->id == landmark2->id) {
              cout << "Oh no, duplicate landmark observation in frame " <<
  frame->id << ": landmarks " << landmark1->id << " " << landmark2->id << endl;
          }
      }
  }
  */
  ...
  for (const auto& observation : frame->keyPointObservations) {
    ...
    // 388.42480338, 274.84471313)); newer college
    Cal3_S2::shared_ptr K(new Cal3_S2(215.69369557, 215.12480881, 0.0,
                                      213.7203901, 119.26347434));
    ...
    if (existingFactorIt != smartFactors.end()) {
      ...
      if (landmark->keyPointObservations.size() > 2) {
        smartFactor =
            SmartFactor::shared_ptr(new SmartFactor(measurementNoise, K));
        ...
        for (const auto& landmarkObservation : landmark->keyPointObservations) {
          Point2 point(landmarkObservation->keyPoint.pt.x,
                       landmarkObservation->keyPoint.pt.y);
          ...
          if (landmarkObservationFrame) {
            ...
            cout << "Failed to lock landmarkObservation->frame! This can "
                    "possibly lead to a indeterminate system."
                 << endl;
          }
        }
      }
  ...
  auto lastPoseDelta =
      isam2.calculateEstimate<Pose3>(frame->id - 2)
          .between(isam2.calculateEstimate<Pose3>(frame->id - 1));

  auto motionPredictedPose =
      lastPoseDelta.compose(isam2.calculateEstimate<Pose3>(frame->id - 1));
...
Pose3 Smoother::updateBatch(const shared_ptr<Frame>& frame) {
  ...
  // Implicitly defines the scale in the scene by specifying the distance
  // between the first two poses
  ...
  if (frame->id == 2) {
    ...
    auto motionPredictedPose =
        lastPoseDelta.compose(estimate.at<Pose3>(frame->id - 1));
...
void Smoother::initializeFirstTwoPoses(const shared_ptr<Frame>& firstFrame,
                                       const shared_ptr<Frame>& secondFrame) {
  Values values;
  auto noise = noiseModel::Diagonal::Sigmas(
      (Vector(6) << Vector3::Constant(0.1), Vector3::Constant(0.3)).finished());
