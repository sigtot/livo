...
def quat2dcm(quaternion):
    """Returns direct cosine matrix from quaternion (derp format, [w x y z])
    """
    q_derp = quaternion[:4]
    q_hamilton = [q_derp[1], q_derp[2], q_derp[3], q_derp[0]]  # [x y z w]
    q = np.array(q_hamilton, dtype=np.float64, copy=True)
    nq = np.dot(q, q)
...
def dcm2quat(matrix_3x3):
    """Return quaternion (derp format, [w x y z]) from rotation matrix.
    ...
    if t > 0.0:
        ...
        q[j] = (M[i, j] + M[j, i])*t
        q[k] = (M[k, i] + M[i, k])*t
    q_derp = np.array([q[3], q[0], q[1], q[2]])  # [w x y z]
    return q_derp
...
if __name__ == '__main__':
    ...
    delimiter = ','
    for i in range(N):
        file_lines.append(''.join([str(timestamp[i]), delimiter,
                                   str(transformed_positions[i, 0]), delimiter,
                                   str(transformed_positions[i, 1]), delimiter,
                                   str(transformed_positions[i, 2]), delimiter,
                                   str(transformed_quats[i, 0]), delimiter,
                                   str(transformed_quats[i, 1]), delimiter,
                                   str(transformed_quats[i, 2]), delimiter,
