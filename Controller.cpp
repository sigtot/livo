...
Controller::Controller(FeatureExtractor& frontend, Smoother& backend,
...
void Controller::imageCallback(const sensor_msgs::Image::ConstPtr& msg) {
  ...
  if (callbackCount == 1) {
    ...
    backend.initializeFirstTwoPoses(twoFirstFrames.first,
                                    twoFirstFrames.second);
  }

  if (callbackCount > 1) {
    // TODO: Fails with IndeterminateLinearSystemException near variable 2.
    // Figure out why.
